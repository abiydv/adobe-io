...
def aws_ses_send(to="", sub="", body=""):
    ...
    try:
        ...
        response = ses.send_email(Source=EMAIL_FROM, Destination=to, Message=message, ReplyToAddresses=[EMAIL_FROM])
    except botocore.exceptions.ClientError as cerr:
        print('error_message: {}'.format(cerr))
        return None
        ...
        print("{} - {}".format(response['ResponseMetadata']['HTTPStatusCode'], response['MessageId']))
        return True

def bad_request():
    return {"statusCode": 400, "headers": {"content-type": "application/json"}, "body": "Bad Request"}
