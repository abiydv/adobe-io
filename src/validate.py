...
def adobe_challenge_request(event):
    return bool(
        "queryStringParameters" in event.keys()
...
def adobe_incoming_headers(event):
    if not event.get("headers"):
        ...
        not event["headers"].get("x-adobe-event-code")
...
def comparehmac(actual, body, key):
    generated = base64.b64encode(
        hmac.new(
            bytes(key, encoding="ascii"), bytes(body, encoding="ascii"), hashlib.sha256
        ).digest()
    ).decode()
    print(
        "info: \n generated {} \n actual {} \n match? : {}".format(
            generated, actual, hmac.compare_digest(actual, generated)
        )
    )
