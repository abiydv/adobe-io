...
def test_aws_ses_send_unverified_email_fail():
    assert (
        src.helper.aws_ses_send(to={"BccAddresses": ["to"]}, sub="sub", body="text")
        is None
    )
...
def test_aws_ses_send_success():
    ...
    assert src.helper.aws_ses_send(
        to={"BccAddresses": ["to@example.com"]}, sub="test", body="test email body"
    )
